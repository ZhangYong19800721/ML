...
from learn.optimal.xrange import xrange
from learn.optimal.gold import gold
...
def minimize_cg(f, x0, p=None):
    ...
    if 'reset' not in p:  # 给出参数但是没有给出reset
        p['reset'] = 500
        print('reset参数，将使用默认值%f', p['reset'])

    if 'gold' not in p:  # 给出参数但是没有给出gold
        p['gold'] = None
    ...
    for it in range(int(p['max_it'])):
        ...
        if (it % p['reset'] == 0 and it > 0) or (y1 <= y2):  # 到达重置点或d1方向不是一个下降方向
            ...
            g2 = f.gradient(x2)
            d2 = -g2
            ng2 = np.linalg.norm(g2)  # 迭代到新的位置x2，并计算函数值、梯度、搜索方向、梯度模
            x1, d1, g1, y1, ng1 = x2, d2, g2, y2, ng2
            print('目标函数:%f 迭代次数:%d 梯度模:%f ' % (y1, it, ng1))
            continue

        g2 = f.gradient(x2)
        ng2 = np.linalg.norm(g2)  # 计算x2处的梯度和梯度模
